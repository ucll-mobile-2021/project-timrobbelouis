...
class SearchIngredientRecipeActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        searchButton.setOnClickListener {
            val input = findViewById<EditText>(R.id.input_search).text.toString()
            doit(input)
        }
    }
    ...
    fun doit(input : String){
        Thread {
            val result = URL("https://api.spoonacular.com/recipes/findByIngredients?apiKey="+ apikey +"&number=10&ingredients=${input}").readText()
